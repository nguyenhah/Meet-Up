...
public class Schedule {
    ...
    public ArrayList<BreakTime> getStartTimesOfBreaksMod(String dayOfWeek) {
        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        ArrayList<BreakTime> startTimes = new ArrayList<>();
        if (sections.size() == 1) {
            Section section = sections.first();
            CourseTime courseTime = section.getCourseTime();
            startTimes.add(new BreakTime(courseTime.getEndTime(), "23:59"));
            //startTimes.add(courseTime.getEndTime());
        } else if (sections.size() > 1) {
            Iterator<Section> it = sections.iterator();
            Section section = it.next();
            ...
            while (it.hasNext()) {
                ...
                if (calculateBreakTimeInMinutes(nextTime, lastTime) >= 60) {
                    //startTimes.add(lastTime);
                    BreakTime b = new BreakTime(lastTime, nextTime);
                    startTimes.add(b);
                    System.out.println(b.getStartBreak()+":"+b.getLengthOfBreak());
                }
